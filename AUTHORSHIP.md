...
| `chord.h`, `chord.cpp` | **A** | ⚠️ | Started 12 Sep 2026, Phase 2. Identifier arithmetic (2.1a), ring construction (2.1b), and routing — `closest_preceding_finger` and `route_step` (2.2, 13 Sep) — all walked through in chunks. **2.2 not yet checked by questioning.** **Comprehension checks on 2.1b answered 2/2 on 13 Sep 2026:** why `successor_of` returns by value (the list dies by scope, so a stored reference would dangle), and why the test oracle must be a different algorithm. The 2.1a interval test has not yet been checked by questioning. |
| `test_chord.cpp` | **A** | ⚠️ | In-process tests over hand-built rings, mutation-checked rather than trusted. From 2.2: a distance-based oracle for finger choice, and a routing simulation checking progress on every hop. Walked through. The reason for an independent oracle was explained back correctly on 13 Sep 2026. |
...
| `test_net.cpp`, `scripts/e2e-node.sh` | **A** | ⚠️ | Framing tests over a socketpair, and a real 8-node ring checked against a Python oracle that shares no code with `chord.cpp`. From 2.2, also an independent Python model of the finger rule: hop counts must match it exactly. Both mutation-checked. Walked through; not yet checked by questioning. |
